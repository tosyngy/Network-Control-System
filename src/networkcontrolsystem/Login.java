...
public class Login extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel3.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0)), "User Confirmation Page"));
        jPanel1.add(jLabel3);
    ...
    void userLogin() {
        if (jTextField1.getText().isEmpty()) {
            jp.showMessageDialog(rootPane, "Username require", "Network Access GateWay", jp.ERROR_MESSAGE);
            jTextField1.requestFocus();
            return;
        }
        if (jPasswordField1.getText().isEmpty()) {
            jp.showMessageDialog(rootPane, "Password require","Network Access GateWay", jp.ERROR_MESSAGE);
            jPasswordField1.requestFocus();
            return;
        }
        ...
        try {
            String sqlQuery = "select password as pass,status from access where username='" + jTextField1.getText() + "'";
            ...
            while (db.rs.next()) {
                if (db.rs.getString("pass").equals(jPasswordField1.getText())) {
                    ...
                    var.setAgentId(db.rs.getInt("status"));
                    log("user access network");
                    if(var.getAgentId()==0){
                    JOptionPane.showMessageDialog(rootPane, "You are not granted access to\nthis network check back later");
                   
                    }else if(var.getAgentId()==1){
    ...
    void log(String event) {
        try {
            ...
            var.setSysId(sc.getHDDSelNum(ip));
            var.setIP(ip);
            Date d = new Date();
            db.ps = db.con.prepareStatement("INSERT INTO `log` (`sysid` ,`ip`  ,`status`)VALUES (?,?,?) ");
